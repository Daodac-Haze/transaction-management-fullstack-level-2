...
describe("Transaction Management Frontend - Level 2", () => {
  it("The app can submit new transactions and show the historical ones", () => {
    ...
    cy.get("[data-type=transaction-submit]").click();
    cy.get(
      `[data-type=transaction][data-account-id=${anotherAccountId}][data-amount=${negativeAmount}][data-balance=${negativeBalance}]`
    ).should("exist");
  });
  ...
  it("The app can handle invalid user input", () => {
    ...
    cy.get(
      `[data-type=transaction][data-account-id=${invalidAccountId}][data-amount=${invalidAccountIdAmount}]`
    ).should("not.exist");
    ...
    cy.get("[data-type=transaction-submit]").click({ force: true });
    cy.get(
      `[data-type=transaction][data-account-id=${invalidAmountAccountId}][data-amount=${invalidAmount}]`
    ).should("not.exist");
  });
});
